...
try:
    ...
    from utils import load_and_parse_npz, translate_coordinates
...
def _worker(args):
    f, Pk, r_k, th_k, ph_k, N, st_db, mx_db, lam, base_norm = args
    coeffs, metrics = _solve_one_frequency(
        f_hz=f, P_complex=Pk, coords_sph=(r_k, th_k, ph_k), order_N=N,
...
def run_open_branch_optimizer(
    ...
    test_indices = np.linspace(idx_target[0], idx_target[-1], 8, dtype=int)

    def run_batch(configs):
        tasks = []
        for k in test_indices:
            f = f_all[k]
            Pk = np.conj([data_dict[fn][k] for fn in filenames])
            ...
            for N, st_db, mx_db, lam in configs:
                tasks.append((f, Pk, r_k, th_k, ph_k, N, st_db, mx_db, lam, base_norm))
    ...
    # =========================================================================
    # STEP 1: FINDING THE TIPPING POINT (Secant Line "Knee" Detection)
    # =========================================================================
    print("\n" + "="*65)
    print(" STEP 1: FINDING THE TIPPING POINT (UNREGULARIZED)")
    print("="*65)
    ...
    # Store data for knee calculation
    n_vals = []
    ratio_vals = []
    prev_ratio = None
    ...
    # --- Secant Line Math ---
    n_start, r_start = n_vals[0], ratio_vals[0]
    n_end, r_end = n_vals[-1], ratio_vals[-1]
    
    # Calculate slope (m) and intercept (c) of the secant line
    m = (r_end - r_start) / (n_end - n_start)
    c = r_start - m * n_start
    
    max_distance = -float('inf')
    tipping_N = n_vals[-1]
    
    for n, r in zip(n_vals, ratio_vals):
        # Expected ratio if the decay was perfectly linear
        expected_r = m * n + c
        distance = r - expected_r
        
        # The knee is the point furthest above the linear decay line
        if distance > max_distance and n != n_start and n != n_end:
            max_distance = distance
            tipping_N = n

    print("-" * 65)
    print(f"=> Secant Line Knee Detected at N={tipping_N} (Max Distance: {max_distance:.2f} dB)")
    
    stable_N = max(1, tipping_N - 1)
    unstable_N = tipping_N + 1
    active_orders = [stable_N, tipping_N]
    step2_orders = [tipping_N, unstable_N]
    
    print(f"=> Keeping Orders N={stable_N} (Stable) and N={tipping_N} (Tipping) active for Step 3.")
    ...
    # =========================================================================
    print("\n" + "="*65)
    print(f" STEP 2: MAPPING NOISE FLOOR (Orders {tipping_N} & {unstable_N}, Lam=0.01)")
    print("="*65)
    ...
    print(f"{'Start dB':<10} | {'Ratio N=' + str(tipping_N):<15} | {'Ratio N=' + str(unstable_N):<15}")
    print("-" * 45)
    
    peak_ratio_unstable = -float('inf')
    best_st_db = -30.0
    
    for st in start_dbs:
        r_tipping = res_s2[(tipping_N, st, st - test_db_transition_span, 0.01)]['ratio_db']
        r_unstable = res_s2[(unstable_N, st, st - test_db_transition_span, 0.01)]['ratio_db']
        
        flag = ""
        # We define the plateau/peak using the unstable order, as it magnifies the noise
        if r_unstable > peak_ratio_unstable:
            peak_ratio_unstable = r_unstable
            best_st_db = st
            flag = " * PEAK (Unstable N)"
            
        print(f"{st:<10.1f} | {r_tipping:<15.2f} | {r_unstable:<15.2f} {flag}")

    print(f"\n=> Unstable Order Peak Rejection found at {best_st_db} dB")
