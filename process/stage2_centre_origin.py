...
# Variable N Limit Table (Format: (Up_to_Frequency_Hz, Order_N))
ORDER_N_TABLE = [
    (250.0, 2),
    (500.0, 3),
    (1000.0, 4),
    (4000.0, 5),
    (float('inf'), 6) # Default for anything above highest specified frequency
]
...
def get_order_for_frequency(f_hz):
    """Returns the order N based on the ORDER_N_TABLE limits."""
    for max_freq, order in ORDER_N_TABLE:
        if f_hz <= max_freq:
            return order
    return ORDER_N_TABLE[-1][1]
...
class DynamicBranchSweeper:
    ...
    def process_frequency(self, branch_name, f_hz, f_idx, current_seed, i_step, is_first):
        ...
        phys_context = ([(float(f_hz), float(2*np.pi*f_hz/self.cfg['speed_of_sound']), np.conj(np.array(P_list)))], 
